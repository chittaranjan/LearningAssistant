package com.juleswhite.module5;

import com.juleswhite.module1.LLMProvider;
...
public class MockAnalysisProvider implements LLMProvider {
    ...
    public Result generateCompletion(Context context) {
        try {
            ...
            switch (step) {
                case 0:
                    response.put("tool", "readPdfContent");
                    args.put("filePath", "curriculum.pdf");
                    response.put("args", args);
                    step++;
                    break;
                case 1:
                    response.put("tool", "readPdfContent");
                    args.put("filePath", "resume.pdf");
                    response.put("args", args);
                    step++;
                    break;
                case 2:
                    response.put("tool", "evaluateCurriculum");
                    args.put("curriculumText", "Extracted content from curriculum.pdf showing MSc AI...");
                    response.put("args", args);
                    step++;
                    break;
                case 3:
                    response.put("tool", "evaluateResume");
                    args.put("resumeText", "Extracted content from resume.pdf showing Software Engineer...");
                    response.put("args", args);
                    step++;
                    break;
                case 4:
