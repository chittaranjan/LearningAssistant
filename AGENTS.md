...
The core logic is located in `backend/src/main/java/com/learningAssistant/core/`:
...
1.  Define a new tool in `AnalysisTools.java` using the `@RegisterTool` annotation.
2.  Add a new `Goal` to the agent's task list in `LocalJavaAgenticService.java`.
3.  (Optional) Update the `MockAnalysisProvider` to simulate the new tool's behavior for testing.
